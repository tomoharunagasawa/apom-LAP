...
先進的な組織は、@<w>{project-centric}から@<w>{product-centric}のモデルへと移行し、チームと投資を顧客のアウトカムに合わせて整合させている。この進化は、組み込み型で、適応力があり、@<w>{outcome-focused}な@<w>{procurement}の職能を求めている。
...
アジャイルプロダクトオペレーティングモデル（APOM: Agile Product Operating Model）は、組織が価値を提供する方法を、@<w>{project-based}から@<w>{product-based}の提供へと転換することで再構成するためのアプローチである。APOMは、@<w>{Strategy}（Strategy）、@<w>{People}（People）、@<w>{Structure}（Structure）、@<w>{Value Cycle}（Value Cycle）という相互に依存する4つの柱を用いて、プロダクトを中心に組織を構成するための@<w>{blueprint}を提供する。
...
グローバルにおけるビジネス環境は、根本的な転換を遂げている。組織の82％が、短期的なニーズと長期の戦略的変革のバランスを取ることに課題を抱えていると回答している@<bib>{2}。この迅速な対応と長期的な計画との間に生じる緊張関係は、従来のアプローチでは対応できない@<w>{procurement}の職能への新たな要求を生み出している。
...
これは、Netflixの「プロセスよりも人を重視する（People over Process）@<bib>{3}」という文化によって実現できた。この文化は、透明性と説明責任を重視し、成功するアウトカムの達成に向けた責任を明確にしている。信頼、裁量、説明責任のバランスが、Netflixにおける@<w>{procurement}プロセスの効率化を可能にしたのである。
...
貿易形態の変化や関税、市場動向の影響が、@<w>{procurement}および@<w>{supply}の状況に影響を与えている。@<w>{supply}とコストの不確実性が高まる中、成功するビジネスアウトカムを維持するためには、契約のより頻繁な見直しと再評価が求められている。サプライチェーンを動的で柔軟かつレジリエンスのあるものとして保つことが、迅速に組織を適応させる鍵となる。

===[/column]

//footnote[e-procurement][訳注: @<w>{e-procurement}（e-procurement）とは、インターネットや専用のオンラインシステムを利用して、企業が商品やサービスを調達するプロセスを指す。これにより、調達プロセスの効率化、コスト削減、透明性の向上が実現される。]

#@# == The Hidden Cost of Procurement and Product Disconnect
== @<w>{procurement}とプロダクトの断絶がもたらす隠れたコスト
...
組織全体に@<w>{procurement}を組み込むことで、業績向上の大きな可能性が生まれる。これにより、@<w>{supplier}を含むパートナーエコシステムの変化を計画的に捉え、契約上の支援を行い、リスクを管理しながら、より迅速なイノベーションを可能にする能力が生まれる。
...
@<wb>{procurement}@<b>{の断絶}: 現在の@<w>{procurement}アプローチは、変更が最小限であり、状況が非常に予測可能であるという根本的な前提に基づいている。しかし、複雑な状況に予測的な@<w>{procurement}プロセスを適用し続けることは、過度な遅延や引き継ぎを引き起こし、プロダクト組織の対応能力を阻害し続けることになる。@<w>{procurement}が@<w>{cross-functional}チームの一員として関与し、統合されることにより、チーム全体で協力できる@<w>{supplier}を見つけるための整合性がとれるようになる。このパートナー重視の姿勢は、相互にWin-Winの関係性をもたらす組織間ネットワークやエコシステムを形成する。これにより、契約の柔軟性をプロセスに組み込むことでリスクが軽減され、可監査性や@<w>{supplier}のレジリエンスが向上する@<bib>{6}。
...
@<b>{市場投入の遅延による損失}: 6〜12か月を要するRFPプロセスの複雑さが、組織が市場機会を活かすことを妨げている。テクノロジーや消費財などの変化が激しい分野では、この遅延が市場でのリーダーになるか、取り残されるかの明暗を分けることになる。組織が数か月かけて@<w>{supplier}を評価している間に、アジャイルな@<w>{procurement}能力を持つ@<w>{competitor}は、すでに顧客に価値を提供していることを考えてみてほしい。
...
プロダクトチームがスプリントで作業している場合、サービスやツールへのアクセスが必要になると、12か月に及ぶRFPプロセスが進行している間、開発が停止されてしまうことがある。これでは、イノベーションの推進が極めて困難になる。チームが2週間スプリントで作業しており、1スプリントあたりのコストが7万ドルであるとすると、12か月間の遅延コストは1チームあたり182万ドルに達する。複数のチームを抱える大規模な組織では、このコストはさらに膨れ上がる。さらに、契約締結後の変更管理にも追加コストが発生し、機会損失に拍車をかけることになる。
...
@<b>{イノベーションの停滞}: 硬直した仕様や取引型のベンダー関係は、@<w>{collaborative}イノベーションや価値共創を阻害することになる。@<w>{supplier}との@<w>{collaboration}は、もはやコスト削減や契約交渉のためだけのものではない。それは、イノベーションを推進し、レジリエンスを確保し、企業とその@<w>{supplier}との双方に長期的な価値を生み出すことに関わるものである。@<w>{supplier}をイノベーションのパートナーではなく、単なる@<w>{order-takers}として扱う組織は、画期的なソリューションを得る機会を逃している。
...
プロダクトを重視する企業は、自社の中核となる提供価値を明確に理解し、自らが活動する状況を把握している。自社で構築するか、それとも外部から調達するかという議論は、より頻繁に行われ、中核ではないニーズは@<w>{procurement}によって外部に委ねられることになる。つまり、柔軟で迅速に対応できる@<w>{procurement}プロセスを持つことが、変化の激しいプロダクトモデルにおいて中核的な能力となるのである。
...
@<b>{アジリティの制約}: 固定された契約は、変化するビジネスニーズや市場状況へのタイムリーな適応を妨げる。経験主義に基づく学習モデルでは、頻繁な提供と変化が求められるため、契約の中に変更制約を組み込むと、調整に時間的コストや金銭的コストが発生してしまう。すべての関係者が共同で作成し、頻繁な見直しと更新を前提とした柔軟な契約やアジャイルな契約は、プロダクトチームとパートナーが変化に対して大きな摩擦を伴わずに対応できる環境を整えるのに役立つ。
...
@<wb>{procurement}@<b>{への影響}: @<w>{procurement}のガバナンスは、@<w>{control-based}のアプローチから@<w>{enablement-based}のアプローチへと転換しなければならない。中央集権的な@<w>{procurement}チームは、プロダクトチームが活動するための制約を提示する役割を担う。この転換には、新しい契約モデル、効率化した承認プロセス、迅速な意思決定を支援する@<w>{collaborative}なツールが求められる。これにより、初期段階からの関与と透明性の向上を通じて、コンプライアンスと可監査性の両方が改善される。
...
プロダクトチームに@<w>{procurement}の能力を組み込むことは、戦略的な@<w>{enabler}である。25年前、プロダクト、QA、オペレーションがそれぞれ独立した部門だったが、価値提供の柔軟性を最大化するためにプロダクトチームへ統合されているのと同じように、主要なビジネス機能すべてを含むように@<w>{cross-functional}的な連携を拡張することで、リスク管理やコンプライアンスを改善しつつ、タイムリーな意思決定を強化できるようになる。
